...
ASHN now has a working EDI intake boundary that accepts canonical XML/JSON payloads plus first-pass raw X12 `834`/`820`/`270`/`269`/`276`/`278`/`837`/`837D`/`835`/`275`, validates them, converts them into ASHN's internal transaction model, audits every submission, and forwards accepted work to `payer-core`.
...
Important nuance: real X12 is often exchanged as delimiter-based EDI text rather than XML. Many enterprise systems also use XML wrappers, canonical XML, or XML-based integration contracts around EDI workflows. ASHN now uses XML/JSON for readable canonical demos and a small raw X12 parser for `834`, `820`, `270`, `269`, `276`, `278`, `837`, `837D`, `835`, and `275` segment intake.
...
- [x] Parse raw X12 `269` benefit coordination into canonical coordination requests.
...
- [x] Add dashboard sample loaders for accepted raw `269` benefit coordination, `837D` dental claims, and `275` documentation attachments.
...
- [x] Preserve the app's current claim/auth attachment path while adding a closer `006020X314` envelope shape.
...
- [x] Add a partner guide coverage summary that compares allowed transaction types and profile-backed validation coverage.
...
- [x] Add a stricter Obsidian Claims companion-guide variant for professional `837` workflows with exact ASHN procedure codes and non-catastrophic diagnosis constraints.
...
- [x] Add an actionable raw X12 sample guide that explains expected green-path outcomes and loads demo payloads directly.
- [x] Add dashboard boundary notes for valid but out-of-scope X12 sets such as `101`, `110`, `201`, `210`, and `215`.
