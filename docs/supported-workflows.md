...
| Premium payment | `820` | `POST /v1/premium-payments`, `GET /v1/premium-payments`, `GET /v1/premium-payments/{id}/export`, XML `820`, raw X12 `820` | Workflow card, premium ledger drilldown/export, timeline, raw X12 form | Records sponsor/member premium dues; history shows reconciled and benefit-current signals that influence claim adjudication. |
...
| Benefit coordination | `269` | `POST /v1/benefit-coordination`, XML `269`, raw X12 `269` | Ledger, timeline, raw X12 form | Captures primary/secondary payer context for COB learning scenarios. |
| Prior authorization | `278 → 275` | `POST /v1/auth-requests`, `POST /v1/auth-requests/{id}/attachments`, `POST /v1/auth-requests/{id}/decision`, XML `278`, raw X12 `278`, XML `275` | Workflow card, auth documentation workbench, manual review widget, ledger, timeline, raw X12 form | Starts pending; supporting 275 documentation can attach and be reviewed before manual/worker decision. Raw dental `278`s preserve CDT, tooth, surface, quadrant, and orthodontic detail. |
...
| Payment/remittance | `835` | `POST /v1/claims/{id}/payment`, XML `835`, raw X12 `835` | Workflow card, claims panel, ledger, detail drawer, raw X12 form | Includes allowed, paid, adjustment, denial fields, with premium-current context from accepted `820`s and dental benefit-plan caps. |
...
- The raw X12 sample guide explains the expected accepted outcome and learning focus for each green-path payload and can load those payloads directly into the intake text area.
