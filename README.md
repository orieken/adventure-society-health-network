...
The dashboard Workflow tab also includes executable and exportable demo scenarios. Each scenario can run a repeatable workflow, stream step results into Live Session Events, save recent runs in-browser, download a versioned JSON runbook, and export a JSON evidence bundle with step results, transaction IDs, and artifact hints for stakeholder walkthroughs.
