...
The dashboard Workflow tab also includes executable and exportable demo scenarios. Each scenario can run end-to-end or in guided playback mode, stream step results into Live Session Events, save recent runs in-browser, download a versioned JSON runbook, and export a JSON evidence bundle with step results, transaction IDs, and artifact hints for stakeholder walkthroughs.
